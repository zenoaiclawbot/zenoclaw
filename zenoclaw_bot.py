...
Compatible with python-telegram-bot v20.7
"""
...
async def run_analysis(bot: Bot):
    ...
    try:
        ...
        if tradeable and vol_ok and mom_ok:
            ...
            log.info("✅ TRADE SIGNAL — broadcasting")
        else:
            reasons = []
            ...
            log.info(f"⚠️ ALERT — broadcasting")
...
async def main():
    ...
    async with app:
        ...
        log.info("ZenoClaw started — polling for commands")

        scheduler = AsyncIOScheduler()
        scheduler.add_job(run_analysis, "interval", minutes=CHECK_EVERY,
                          args=[app.bot], id="zenoclaw")
        scheduler.start()
        log.info(f"Scheduler started. Subscribers: {len(subscribers)}")

        await run_analysis(app.bot)
